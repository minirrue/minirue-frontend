...
function ProductCard({ product, index = 0, onClick, traceIdPrefix }: ProductCardProps) {
  ...
  return (
    ...
    >
      ...
      {/* Wishlist — shown on hover (desktop) or revealed (touch). A sibling of
          the <Link>, not a descendant: a <button> can't legally sit inside an
          <a>, and the tile it visually overlaps is this root's first child, so
          absolute-positioning from here lands on the same pixels as before. */}
      <div
        style={{
          position: 'absolute',
          top: 12,
