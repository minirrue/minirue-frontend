...
async function fillGuestBasics(user: ReturnType<typeof userEvent.setup>, governorate: string) {
  fireEvent.change(screen.getByLabelText(/full name/i), { target: { value: 'Volta Joe' } });
  fireEvent.change(screen.getByLabelText(/^email$/i), { target: { value: 'volta@example.com' } });
...
describe('checkout delivery step — Standard / Same-day (frontend#163)', () => {
  ...
  it('resolves a pasted Google Maps link as the location when no pin is dropped', async () => {
    ...
    fireEvent.change(screen.getByLabelText(/paste a google maps link/i), {
      target: { value: 'https://maps.google.com/maps?q=30.05,31.24' },
    });
