...
const INITIAL_STATE: ScrollDirectionState = { direction: 'up', atTop: true, y: 0 };
...
export function useScrollDirection(threshold = DEFAULT_THRESHOLD): ScrollDirectionState {
  ...
  React.useEffect(() => {
    ...
    setState({ direction: 'up', atTop: window.scrollY <= TOP_ZONE_PX, y: window.scrollY });
    ...
    const sample = () => {
      ...
      if (atTop) {
        lastY.current = y;
        setState((prev) =>
          prev.atTop && prev.direction === 'up' && prev.y === y
            ? prev
            : { direction: 'up', atTop: true, y },
        );
        return;
      }
      ...
      if (Math.abs(delta) < threshold) {
        // Below threshold: direction doesn't flip, but keep `y` fresh so
        // callers doing their own math (e.g. a fade tied to scroll position)
        // aren't stuck on a stale value.
      ...
      lastY.current = y;
      setState((prev) =>
        prev.direction === direction && !prev.atTop && prev.y === y
          ? prev
          : { direction, atTop: false, y },
      );
    };
